package com.qfedu.elasticsearch.transport;
...
public class TransportBean {
    ...
    public boolean save(String indexName, String type, String id, String data){
        ...
        //新增数据
        IndexResponse response = client.prepareIndex(indexName,type,id).setSource(
    ...
    public boolean saveBatch(String indexName, String type, Map<String,String> data){
        ...
        for (String id: data.keySet()) {
            bulkRequestBuilder.add(client.prepareIndex(indexName,type,id).setSource(
                    data,XContentType.JSON
            ));
        }
    ...
    public boolean update (String indexName, String type, String id, String data){
        //修改数据
        UpdateResponse response = client.prepareUpdate(indexName, type, id).setDoc(
                data,XContentType.JSON
        ).get();
        return response.status().getStatus() > 0;
    }
    ...
    //查询某个字段的内容对应的数据
    public String queryByMsf(String indexName, String type, String field, String msg){
        //创建字段和值的对应查询
    ...
    public List<String> queryLike(String indexName, String type, String field, String msg, int index, int count){
        //创建字段和值的对应查询
        WildcardQueryBuilder queryBuilder = QueryBuilders.wildcardQuery(field,"*"+ msg +"*");
        //查询条件构建器
        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
        //设置对应的查询条件
